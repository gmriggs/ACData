    ...
    public static class LifestonedConverter
    {
        ...
        public static bool TryConvert(global::Lifestoned.DataModel.Gdle.Weenie input, out Weenie result, bool correctForEnumShift = false)
        {
            ...
            try
            {
                ...
                else if (input.WeenieClassId <= ushort.MaxValue && WeenieClassNames.TryGetValue(input.WeenieClassId, out var classname))
                {
                    var clsName = classname.ToLower().Substring(2);
        ...
        public static bool TryConvertACEWeenieToLSDJSON(Weenie weenie, out string result, out global::Lifestoned.DataModel.Gdle.Weenie json_weenie)
        {
            try
            {
                if (TryConvert(weenie, out json_weenie))
                {
                    try
                    {
                        result = JsonConvert.SerializeObject(json_weenie, SerializerSettings);
                    }
                    catch
                    {
